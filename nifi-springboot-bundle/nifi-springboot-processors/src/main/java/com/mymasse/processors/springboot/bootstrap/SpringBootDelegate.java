...
public class SpringBootDelegate implements Closeable, SpringDataExchanger {
    ...
    private SpringBootDelegate(String className) {
        ...
        try {
            this.applicationContext = SpringApplication.run(className, new String[] {});
            if (this.applicationContext.containsBean(SpringNiFiConstants.FROM_NIFI)) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Spring Boot Application '" + className + "' is capable of receiving messages from NiFi since 'fromNiFi' channel was discovered.");
                }
            } else {
            ...
            if (this.applicationContext.containsBean(SpringNiFiConstants.TO_NIFI)) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Spring Boot Application '" + className + "' is capable of sending messages to " + "NiFi since 'toNiFi' channel was discovered.");
                }
            } else {
                this.fromSpringChannel = null;
            }
            if (logger.isInfoEnabled() && this.toSpringChannel == null && this.fromSpringChannel == null) {
                logger.info("Spring Boot Application '" + className + "' is headless since neither 'fromNiFi' nor 'toNiFi' channels were defined. No data will be exchanged.");
            }
        } finally {
