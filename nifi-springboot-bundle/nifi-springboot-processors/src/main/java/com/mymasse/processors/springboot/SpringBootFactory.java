...
public final class SpringBootFactory {
    ...
    static SpringDataExchanger createSpringBootDelegate(String springBootClass, String springBootJarFile) {
        ...
        SpringBootClassLoader contextCl = new SpringBootClassLoader(urls.toArray(new URL[] {}), SpringBootFactory.class.getClassLoader());
        ...
        try {
            ...
            SpringDataExchanger springDelegate = (SpringDataExchanger) ctr.newInstance(springBootClass);
            ...
            throw new IllegalStateException("Failed to instantiate Spring Boot Application. Class: '" + springBootClass + "'; Classpath: " + Arrays.asList(urls), e);
        } finally {
    ...
    static List<URL> gatherClassPathUrls(String bootJarPath) {
        ...
        if (bootJarPathFile.exists() && bootJarPathFile.isFile() && bootJarPathFile.getName().toLowerCase().endsWith(".jar")) {
            try {
            ...
            throw new IllegalArgumentException("Path '" + bootJarPathFile.getAbsolutePath() + "' is not valid because it doesn't exist or does not point to a jar file.");
        }
    }
